...
def _get_top_students_and_popular_tryouts(db: Session, completed_attempts=None, tryout_map=None, results_by_attempt=None, users_by_id=None):
    if completed_attempts is None:
        completed_attempts = db.query(Attempt).filter(Attempt.status != "IN_PROGRESS").all()
    if tryout_map is None:
        tryout_map = {t.id: t for t in db.query(Tryout).all()}
    if results_by_attempt is None:
    ...
    return top_students, popular_tryouts
...
def get_admin_dashboard_summary(
    ...
    top_students, popular_tryouts = _get_top_students_and_popular_tryouts(
        db, completed_attempts, tryout_map, results_by_attempt, users_by_id
    )
